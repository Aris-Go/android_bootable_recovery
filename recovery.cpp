...
Device::BuiltinAction start_recovery(Device* device, const std::vector<std::string>& args) {
  ...
  // Extract the YYYYMMDD timestamp from the full version string.
  // Assume the first instance of "-[0-9]{8}-".
  std::string ver = android::base::GetProperty("ro.aris.version", "");
  ...
  std::vector<std::string> title_lines = {
    "Version " + android::base::GetProperty("ro.aris.build.version", "(unknown)") +
        " (" + ver_date + ")",
  };
